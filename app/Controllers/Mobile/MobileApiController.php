...
/**
 * Mobile Android Gateway Client Controller
 * 
 * Implements the standard mobile contract (compatible with com.httpsms)
 * for FCM push tokens, outstanding message queries, message delivery events,
 * and device battery/connectivity heartbeats.
 */
class MobileApiController extends BaseController
{
    ...
    public function updateFcmToken(): ResponseInterface
    {
        ...
        log_message('info', "[MobileApi::updateFcmToken] Incoming request from IP {$this->request->getIPAddress()}: {$rawBodyString}");
        ...
        if (empty($fcmToken) || empty($phoneNumber)) {
            log_message('error', "[MobileApi::updateFcmToken] Validation failed: missing fcm_token or phone_number");
        ...
        log_message('info', "[MobileApi::updateFcmToken] SUCCESS: Registered SIM Line {$phoneNumber} (SIM1) with FCM Token (" . substr($fcmToken, 0, 20) . "...) for User {$userId}");
    ...
    public function getOutstandingMessage(): ResponseInterface
    {
        ...
        log_message('info', "[MobileApi::getOutstandingMessage] Request from IP {$this->request->getIPAddress()} for message_id: '{$messageId}'");

        if (empty($messageId)) {
            log_message('error', "[MobileApi::getOutstandingMessage] Missing message_id parameter");
        ...
        if (!$job) {
            log_message('warning', "[MobileApi::getOutstandingMessage] Message not found for id: '{$messageId}'");
        ...
        log_message('info', "[MobileApi::getOutstandingMessage] Dispatched message payload to mobile: ID {$job['job_id']} (Attempt: {$newAttempt}) -> Device: {$assignedDev}");
    ...
    public function recordMessageEvent(string $messageId): ResponseInterface
    {
        ...
        log_message('info', "[MobileApi::recordMessageEvent] Message {$messageId} event payload from IP {$this->request->getIPAddress()}: {$rawBodyString}");
        ...
        $job = $this->jobModel->findByIdOrClientId($messageId);
        if (!$job) {
            log_message('notice', "[MobileApi::recordMessageEvent] Message {$messageId} not in DB (already deleted or archived). Returning 200 OK.");
            return $this->response->setStatusCode(200)->setJSON([
                'data'    => null,
                'message' => 'ok',
                'status'  => 'success',
        ...
        if ($eventName === 'SENT') {
            ...
            log_message('info', "[MobileApi::recordMessageEvent] Job {$job['job_id']} updated to STATUS_SENT (Device: {$assignedDev}, Attempt: {$currentAttempt})");
            ...
            log_message('info', "[MobileApi::recordMessageEvent] Job {$job['job_id']} updated to STATUS_DELIVERED (Device: {$assignedDev}, Attempt: {$currentAttempt})");
            ...
            log_message('warning', "[MobileApi::recordMessageEvent] Job {$job['job_id']} updated to STATUS_FAILED (Reason: {$reason})");
        }
    ...
    public function recordHeartbeat(): ResponseInterface
    {
        ...
        log_message('info', "[MobileApi::recordHeartbeat] Device '{$deviceId}' Heartbeat from IP {$this->request->getIPAddress()} | Phone: {$primaryPhone} | Carrier: {$carrier} | Battery: {$battery}% (Charging: {$charging})");
    ...
    public function receiveMessage(): ResponseInterface
    {
        ...
        log_message('info', "[MobileApi::receiveMessage] Incoming SMS received from device: {$rawBodyString}");
